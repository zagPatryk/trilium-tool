---
name: trilium-second-brain-windows
description: "Use when Trilium knowledge work runs on Windows."
version: 1.0.0
author: Hermes Agent
license: MIT
platforms: [windows]
metadata:
  hermes:
    tags: [trilium, knowledge, second-brain, windows, agents]
---

# Trilium Second Brain on Windows

## Overview

Use `trilium-kb` as the Windows agent's durable-memory client. Trilium is for reusable analyses, plans, decisions, runbooks, and substantial session outcomes. Git remains canonical for code and repository documentation.
...
Use this skill on a Windows agent when existing Trilium knowledge may affect substantive work or when the result may be worth revisiting. Do not load it on another operating system or create knowledge for a trivial task.
...
py -m pipx install "trilium-tool @ git+https://github.com/zagPatryk/trilium-tool.git"
trilium-kb --help
```
...
Create a different ETAPI token for every device so one Windows machine can be revoked independently. Store it in a local file protected by a Windows ACL. The following values are placeholders; replace them locally, never in a committed file or chat transcript.

```powershell
$ConfigDir = Join-Path $env:LOCALAPPDATA "trilium-tool"
$Config = Join-Path $ConfigDir "device.env"
$Outbox = Join-Path $ConfigDir "outbox"

New-Item -ItemType Directory -Force -Path $ConfigDir, $Outbox | Out-Null
$Identity = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
icacls.exe $ConfigDir /inheritance:r | Out-Null
icacls.exe $ConfigDir /grant:r "${Identity}:(OI)(CI)(F)" | Out-Null

$ConfigText = @"
TRILIUM_URL=https://<trilium-magicdns-name>.<tailnet>.ts.net
TRILIUM_ETAPI_TOKEN=<per-device-etapi-token>
AGENT_KB_OUTBOX=$Outbox
"@
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($Config, $ConfigText, $Utf8NoBom)
icacls.exe $Config /inheritance:r | Out-Null
icacls.exe $Config /grant:r "${Identity}:(F)" | Out-Null

$env:TRILIUM_ENV_FILE = $Config
[Environment]::SetEnvironmentVariable("TRILIUM_ENV_FILE", $Config, "User")
```

Do not use a roaming or repository directory for the outbox. The resulting device-local locations are:

- configuration: `%LOCALAPPDATA%\trilium-tool\device.env`
- outbox: `%LOCALAPPDATA%\trilium-tool\outbox`

Confirm the ACL without printing the file:

```powershell
icacls.exe $Config
[Environment]::GetEnvironmentVariable("TRILIUM_ENV_FILE", "User")
```

Never display the file with `Get-Content`, include it in diagnostics, or copy it into a project. Pass configuration only through `TRILIUM_ENV_FILE`.
...
In PowerShell, write a concise semantic HTML fragment to a temporary file, choose a stable non-secret marker for that one outcome, and run:

```powershell
$HistoryHtml = Join-Path $env:TEMP "trilium-sent-history.html"
@'
<section><h2>Session outcome</h2><p>Durable result and next action.</p></section>
'@ | Set-Content -LiteralPath $HistoryHtml -Encoding utf8

trilium-kb append <sent-history-noteId> `
  --html-file $HistoryHtml `
  --marker <stable-session-outcome-key>
Remove-Item -LiteralPath $HistoryHtml -Force
```
...
2. **Sharing one ETAPI token.** Use a distinct, revocable token for this Windows device.
3. **Weak file permissions.** Remove inherited ACL entries and grant the current identity access before use.
...
- [ ] The file ACL is restricted to the current Windows identity.
- [ ] The URL uses valid HTTPS through Tailscale Serve/MagicDNS or a trusted reverse proxy.
- [ ] The outbox is under `%LOCALAPPDATA%\trilium-tool\outbox`.
